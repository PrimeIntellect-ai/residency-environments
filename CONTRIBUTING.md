...
2. **Develop** your Verifiers v1 taskset under `environments/<env_name>_v1/`
   (see `AGENTS.md` for conventions and how to run things locally).
...
- **Style** (ruff) and **unit tests** run on every PR. They run without any
  secrets, so each changed taskset is installed but its model rollout is skipped.
