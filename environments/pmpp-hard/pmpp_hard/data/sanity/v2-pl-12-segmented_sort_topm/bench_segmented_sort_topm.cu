...
static std::vector<HostCase> build_bench_cases() {
    ...
    cases.push_back(make_case(
        "uniform_N32768_S256_M16",
        32768, 256, 16, SST_DIST_UNIFORM, s++));

    cases.push_back(make_case(
        "powerlaw_N65536_S256_M64",
        65536, 256, 64, SST_DIST_POWERLAW, s++));
    ...
    cases.push_back(make_case(
        "many_ties_N65536_S4096_M16",
        65536, 4096, 16, SST_DIST_MANY_TIES, s++));
...
int main(int argc, char** argv) {
    try {
        ...
        for (const HostCase& hc : host_cases) {
            ...
            std::printf(
                "bench_case %-32s N=%d S=%d M=%d dist=%d\n",
                hc.name.c_str(),
                hc.run.N,
                hc.run.S,
                hc.run.M,
