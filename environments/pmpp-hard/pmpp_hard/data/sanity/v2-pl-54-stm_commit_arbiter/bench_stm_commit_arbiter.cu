...
static std::vector<StepHost> build_seq(const StmProblemSpec& spec) {
    SplitMix64 rng(g_state ^ 0x12345678ULL);
...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<StepHost> host_steps = build_seq(spec);
        ...
        std::printf("bench stm_commit_arbiter max_txns=%d max_locations=%d T=%zu\n",
                    spec.max_txns, spec.max_locations, steps.size());
