...
static std::vector<StepHost> build_bench_steps(const SmtqProblemSpec& spec) {
...
int main(int argc, char** argv) {
    try {
        ...
        const std::vector<StepHost> host_steps = build_bench_steps(spec);
        std::vector<DeviceStep*> steps;
        steps.reserve(host_steps.size());

        for (const StepHost& h : host_steps) {
            steps.push_back(make_device_step(h, spec));
        }

        std::printf(
            "bench_sequence G=%d K=%d bins=%d max_batch=%d steps=%zu\n",
            spec.G,
            spec.K,
            spec.num_bins,
            spec.max_batch,
