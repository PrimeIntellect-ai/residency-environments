...
// replays the same op structure (per-step op counts, op-type sequence, the
// ALLOC/FREE object-id lifetime graph — timing comparability) with fresh
// per-episode op VALUES (ALLOC class/pages, SEAL class, CLEAN budgets) derived
// from (PMPP_BENCH_SEED, episode, step). Warmup uses dedicated episodes. After
// each timed episode (outside the event pair — zero timing impact) a probe
// kernel folds every step's 23 graded u64 outputs into acc[episode]; the
// digest folds every accumulator, so a cached-replay episode cannot match.

struct DeviceStep {
    BacRunSpec run;
    DeviceBuffer<BacOp> ops;
    DeviceBuffer<BacOp> ops_var;    // per-episode variants, episode e at e*n
...
struct C3ProbeTable { C3ProbeDesc d[48]; int n; };
...
static std::vector<StepHost> build_seq(const BacProblemSpec& spec) {
    ...
    SplitMix64 rng(0x1234abcdULL ^ (pmpp::bench_seed(0) * 0x9e3779b97f4a7c15ULL));
...
static DeviceStep* make_device_step(const StepHost& h, int n_ep, uint64_t c3_root) {
    ...
    // Per-episode value variants over the shared op structure.
    const size_t n = h.ops.size();
    std::vector<BacOp> var((size_t)n_ep * n);
    for (int e = 0; e < n_ep; ++e) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            switch (op.op_type) {
                case BAC_OP_ALLOC:
                    op.class_id = r.uniform_int(0, 3);
                    op.a = (uint64_t)r.uniform_int(1, 16);
                    break;
                case BAC_OP_SEAL:
                    op.class_id = r.uniform_int(0, 3);
                    break;
                case BAC_OP_CLEAN:
                    op.a = (uint64_t)r.uniform_int(0, 8);
                    op.b = (uint64_t)r.uniform_int(0, 64);
                    break;
                default:  // FREE: object-id lifetime graph stays fixed
                    break;
            }
            var[(size_t)e * n + i] = op;
        }
    }
    ds->ops_var.allocate(var.size());
    ds->ops_var.upload(var);
...
int main(int argc, char** argv) {
    try {
        ...
        for (auto& h : hs) steps.push_back(make_device_step(h, n_ep, c3_root));

        // Host-side pointer flip between episodes: zero GPU work while timed.
        auto set_episode = [&](int e) {
            for (auto* ds : steps)
                ds->inputs.ops = ds->ops_var.ptr + (size_t)e * ds->ops.count;
        };

        C3ProbeTable pt;
        pt.n = (int)steps.size();
        for (size_t s = 0; s < steps.size(); ++s)
            pt.d[s] = {(const uint8_t*)steps[s]->scalars.ptr, 23 * sizeof(uint64_t)};
        ...
        // step's 23 graded u64 outputs (contiguous per-step scalar block) in order.
        CUDA_CHECK(solution_reset(state, stream));
        for (auto* ds : steps) CUDA_CHECK(solution_run(state,&ds->run,&ds->inputs,&ds->out,workspace.ptr,std::max<size_t>(wb,1),stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));
        ...
        for (auto* ds : steps) dg.dev(ds->scalars.ptr, 23 * sizeof(uint64_t));
        dg.print();
