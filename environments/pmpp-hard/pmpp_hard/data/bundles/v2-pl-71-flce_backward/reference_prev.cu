...
__global__ void k_logits(const uint16_t* X, const uint16_t* W, const uint16_t* bias,
    ...
    int local_row = warp / V;
    int v         = warp % V;
    int xrow = input_base + local_row;
    float p = 0.0f;
    for (int h = lane; h < H; h += 32) {
        float xv = decode_bf16(X[(size_t)xrow * H + h]);
        float wv = decode_bf16(W[(size_t)v * H + h]);
        p = __fadd_rn(p, __fmul_rn(xv, wv));
    }
    float dot = warp_tree_sum(p, lane);
    if (lane == 0) {
        float logit = dot;
        if (has_bias) logit = __fadd_rn(logit, decode_bf16(bias[v]));
...
// ── Kernel: dX[out_row, h] = sum_v dlogit[v] * W[v,h] ────────────────────────
__global__ void k_dx(const uint16_t* W, const int32_t* target, int ignore_index,
...
// ── Kernel: dW accumulate (per chunk, per (v,h)) ─────────────────────────────
__global__ void k_dW(const uint16_t* X, int H, int V, int chunk_rows, int input_base,
                     const float* dlogits_ws, float* acc_dW) {
    int warp = blockIdx.x * WPB + (threadIdx.x >> 5);
    int lane = threadIdx.x & 31;
    int ntasks = V * H;
    if (warp >= ntasks) return;
    ...
    for (int i = lane; i < chunk_rows; i += 32) {
        float dl = dlogits_ws[(size_t)i * V + v];
        float xv = decode_bf16(X[(size_t)(input_base + i) * H + h]);
        p = __fadd_rn(p, __fmul_rn(dl, xv));
    }
    float g = warp_tree_sum(p, lane);
    if (lane == 0) {
        size_t off = (size_t)v * H + h;
        acc_dW[off] = __fadd_rn(acc_dW[off], g);
    }
}
    ...
    for (uint32_t j = 0; j < run->op_count; ++j) {
        ...
        if (oi.kind == 1 && oi.valid) {
            ...
            for (uint32_t c = 0; c < nchunks; ++c) {
                ...
                k_logits<<<(crv + WPB - 1) / WPB, BLK, 0, stream>>>(
                ...
                k_dx<<<(crh + WPB - 1) / WPB, BLK, 0, stream>>>(
                    in->w_bf16, in->target, sp.ignore_index, H, V, chunk_rows,
                    input_base, out_cursor, chunk_begin, dlog_ws, out->dx_bf16);
                int vh = V * H;
                k_dW<<<(vh + WPB - 1) / WPB, BLK, 0, stream>>>(
                    in->x_bf16, H, V, chunk_rows, input_base, dlog_ws, st->d_accW);
                if (sp.has_bias)
