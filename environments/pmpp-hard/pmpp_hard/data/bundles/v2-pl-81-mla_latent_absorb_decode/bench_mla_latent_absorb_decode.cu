...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Weights.
        // Weights and pool payloads derive from PMPP_BENCH_SEED (cache_hash is
        // exact-graded over the stored latents) so a memorized answer table is
        // wrong on a fresh seed; shapes/schedule and RoPE tables stay fixed.
        const uint64_t base_seed = pmpp::bench_seed(0xB81A0BE7C4ULL);
        SplitMix64 wrng(base_seed);
        ...
        // timed iteration replays the same decode SCHEDULE (timing comparable) but on
        // distinct latent/query payloads, and its graded outputs are folded into
        ...
        for (int set = 0; set <= iters; ++set) {
            ...
            for (int v = 0; v < kPools; ++v) {
                SplitMix64 rng(set_seed ^ (0x9F1E0000ULL + (uint64_t)v));
        ...
        for (int iter = 0; iter < iters; ++iter) {
            ...
            dg.dev(d_cache_hash.ptr, (size_t)B * sizeof(uint64_t));
            dg.dev(d_meta.ptr, sizeof(uint64_t));
            dg.dev(d_sat.ptr, sizeof(int32_t));
            dg.dev(d_total.ptr, sizeof(int32_t));
        }
