...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // One INDEPENDENT scenario per timed iteration (plus scenario 0 for warmup).
        // A scenario = {kPools payload pools, kSteps derived target_sig buffers}. Every
        // timed iteration replays the same tree topology and accept/reject SCHEDULE
        // (timing comparable) but on distinct payloads (and the target_sig plan is
        // recomputed from those payloads), and its graded outputs are folded into
        // out_fnv after the iteration. A compute-once-then-replay solution serves stale
        // outputs for iterations 2..K → the folded digest diverges → perf FAIL. Per-set
        // seeds come from a SplitMix64 stream over PMPP_BENCH_SEED so the same seed
        // reproduces the same inputs bit-for-bit.
        const int pattern[8] = {4, 2, 6, 0, 3, 5, 1, 7};
        StvOracleState helper;
        helper.init(spec);

        struct Scenario {
            std::vector<InputPool> pools;
            std::vector<DeviceBuffer<uint64_t>*> d_sigs;
        };

        auto build_scenario = [&](uint64_t set_seed) -> Scenario* {
            Scenario* sc = new Scenario();
            sc->pools.resize(kPools);
            for (int v = 0; v < kPools; ++v) {
                SplitMix64 rng(set_seed ^ (0xA5117E00ULL + (uint64_t)v));
                InputPool& pl = sc->pools[v];
            ...
            // Byte-exact correct signatures per (pool, row, node) for THIS scenario.
                ...
                for (int b = 0; b < B; ++b) {
                    for (int i = 0; i < N; ++i) {
                        const StvOracleToken tok = helper.quantize_token(
                            sc->pools[v].h_draft_k.data() + (((size_t)b * N + i) * Hkv) * D,
                            sc->pools[v].h_draft_v.data() + (((size_t)b * N + i) * Hkv) * D);
            ...
            // Per-step target_sig buffers implementing the (fixed) acceptance plan.
            sc->d_sigs.resize(kSteps);
            ...
            for (int s = 0; s < kSteps; ++s) {
                ...
                sc->d_sigs[s] = new DeviceBuffer<uint64_t>();
                sc->d_sigs[s]->allocate(sig.size());
                sc->d_sigs[s]->upload(sig);
            }
        ...
        const uint64_t base_seed = pmpp::bench_seed(0xA5117E00ULL);
        SplitMix64 set_seed_rng(base_seed ^ 0x700000000ULL);
        std::vector<Scenario*> scenarios((size_t)iters + 1, nullptr);
        for (int set = 0; set <= iters; ++set) {
            scenarios[(size_t)set] = build_scenario(set_seed_rng.next_u64());
        }
        ...
        auto run_sequence = [&](const Scenario& sc) {
            for (int s = 0; s < kSteps; ++s) {
                const int v = s % kPools;
                StvRunSpec run = {};
                ...
                inputs.draft_k = sc.pools[v].draft_k.ptr;
                inputs.draft_v = sc.pools[v].draft_v.ptr;
                inputs.q = sc.pools[v].q.ptr;
                inputs.target_sig = sc.d_sigs[s]->ptr;
                inputs.bonus_k = sc.pools[v].bonus_k.ptr;
                inputs.bonus_v = sc.pools[v].bonus_v.ptr;
        ...
        // they are NOT folded; the exact-graded accept/state/counter outputs are.
        pmpp::OutFnv dg;

        for (int iter = 0; iter < iters; ++iter) {
            const Scenario& sc = *scenarios[(size_t)iter + 1];
            ...
            dg.dev(d_tail.ptr, (size_t)B * sizeof(int32_t));
            dg.dev(d_alen.ptr, (size_t)B * sizeof(int32_t));
            ...
            dg.dev(d_free.ptr, sizeof(int32_t));
            dg.dev(d_allocs.ptr, sizeof(int32_t));
            dg.dev(d_frees.ptr, sizeof(int32_t));
        }
        ...
        for (Scenario* sc : scenarios) {
            for (int s = 0; s < kSteps; ++s) delete sc->d_sigs[s];
            delete sc;
        }
