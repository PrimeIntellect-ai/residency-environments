...
// One sequence variant per timed iteration (plus one for warmup): same seq/count/step
// STRUCTURE every variant (timing comparability), but token VALUES are derived from
// (PMPP_BENCH_SEED, variant). Every timed call thus receives fresh inputs, and each
// iteration's outputs are folded into out_fnv below, so replaying an output cached
// from warmup or an earlier iteration leaves a stale region in the digest → perf FAIL.
static std::vector<StepHost> build_bench_steps(const PrheProblemSpec& spec,
                                               uint64_t variant_seed) {
    SplitMix64 rng(variant_seed ^ 0x55555555ULL);
    ...
    try {
        ...
        for (int v = 0; v <= iters; ++v) {
            std::vector<StepHost> host_steps =
                build_bench_steps(spec, vseed_rng.next_u64());
        ...
        for (int iter = 0; iter < iters; ++iter) {
            ...
            for (DeviceStep* ds : steps) {
                dg.dev(ds->live_count.ptr, (size_t)spec.B * sizeof(int32_t));
                dg.dev(ds->live_sum.ptr, (size_t)spec.B * sizeof(int64_t));
                dg.dev(ds->live_hash.ptr, (size_t)spec.B * sizeof(uint64_t));
                dg.dev(ds->page_table_checksum.ptr, sizeof(uint64_t));
                dg.dev(ds->evicted_count.ptr, sizeof(int32_t));
                dg.dev(ds->free_pages.ptr, sizeof(int32_t));
            }
        }
