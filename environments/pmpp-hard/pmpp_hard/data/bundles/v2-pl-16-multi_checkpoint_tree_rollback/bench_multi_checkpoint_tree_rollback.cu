...
// One sequence variant per timed iteration (plus one for warmup): same op/step
// STRUCTURE every variant (timing comparability), but token/correction VALUES are
// derived from (PMPP_BENCH_SEED, variant). Every timed call thus receives fresh
// inputs, and each iteration's outputs are folded into out_fnv below, so replaying
// an output cached from warmup or an earlier iteration leaves a stale region in
// the digest → perf FAIL.
static std::vector<StepHost> build_bench_sequence(const MctrProblemSpec& spec,
                                                  uint64_t variant_seed) {
    SplitMix64 rng(variant_seed ^ 0x80808080ULL);
...
int main(int argc, char** argv) {
    try {
        ...
        for (int iter = 0; iter < iters; ++iter) {
            ...
            for (DeviceStep* ds : steps) {
                dg.dev(ds->live_cache_sum.ptr, (size_t)spec.B * sizeof(int64_t));
                dg.dev(ds->live_cache_tail_hash.ptr, (size_t)spec.B * sizeof(uint64_t));
                dg.dev(ds->length.ptr, (size_t)spec.B * sizeof(int32_t));
                dg.dev(ds->num_checkpoints.ptr, (size_t)spec.B * sizeof(int32_t));
                dg.dev(ds->top_checkpoint_len.ptr, (size_t)spec.B * sizeof(int32_t));
