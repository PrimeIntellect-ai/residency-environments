# PMPP_CANARY_26_99795d89d8 -- held-out canary; MUST NOT appear in any submission
# file: solution.py

import torch
import triton
import triton.language as tl


@triton.jit
def _paged_decode_attn_kernel(
    q_ptr,
    k_ptr,
    v_ptr,
    page_table_ptr,
    cache_lens_ptr,
    out_ptr,
    part_ptr,
    split_len,
    HQ: tl.constexpr,
    HKV: tl.constexpr,
    MAX_PAGES: tl.constexpr,
    PAGE_SIZE: tl.constexpr,
    D: tl.constexpr,
    SCALE: tl.constexpr,
    WINDOW: tl.constexpr,
    MAX_TOKENS: tl.constexpr,
    NSPLIT: tl.constexpr,
    FINAL: tl.constexpr,
    BLOCK_N: tl.constexpr,
):
    pid = tl.program_id(0)
    sid = tl.program_id(1)
    b = pid // HQ
    hq = pid - b * HQ

    group = HQ // HKV
    hkv = hq // group

    offs_d = tl.arange(0, D)

    q_vec = tl.load(q_ptr + (b * HQ + hq) * D + offs_d).to(tl.float32)

    seq_len = tl.load(cache_lens_ptr + b).to(tl.int64)
    seq_len = tl.minimum(tl.maximum(seq_len, 0), MAX_TOKENS)

    if WINDOW > 0:
        lo = tl.maximum(tl.full((), 0, tl.int64), seq_len - WINDOW)
    else:
        lo = tl.full((), 0, tl.int64)

    lo = tl.maximum(lo, sid.to(tl.int64) * split_len)
    hi = tl.minimum(seq_len, (sid.to(tl.int64) + 1) * split_len)
    if NSPLIT == 1:
        hi = seq_len

    neg = -1.0e30
    m_i = neg
    l_i = 0.0
    acc = tl.zeros((D,), tl.float32)

    for pos in range(lo, hi, BLOCK_N):
        tok = pos + tl.arange(0, BLOCK_N).to(tl.int64)
        in_range = tok < hi

        page_id = tok // PAGE_SIZE
        page_off = tok - page_id * PAGE_SIZE

        phys = tl.load(
            page_table_ptr + b * MAX_PAGES + page_id,
            mask=page_id < MAX_PAGES,
            other=-1,
        ).to(tl.int64)

        valid = in_range & (phys >= 0)
        phys_safe = tl.where(valid, phys, 0)

        kv_off = ((phys_safe[:, None] * HKV + hkv) * PAGE_SIZE + page_off[:, None]) * D

        k_block = tl.load(
            k_ptr + kv_off + offs_d[None, :],
            mask=valid[:, None],
            other=0.0,
        ).to(tl.float32)

        scores = tl.sum(k_block * q_vec[None, :], axis=1) * SCALE
        scores = tl.where(valid, scores, neg)

        block_m = tl.max(scores, axis=0)
        m_new = tl.maximum(m_i, block_m)

        alpha = tl.exp(m_i - m_new)
        p = tl.exp(scores - m_new)
        p = tl.where(valid, p, 0.0)

        v_block = tl.load(
            v_ptr + kv_off + offs_d[None, :],
            mask=valid[:, None],
            other=0.0,
        ).to(tl.float32)

        acc = acc * alpha + tl.sum(p[:, None] * v_block, axis=0)
        l_i = l_i * alpha + tl.sum(p, axis=0)
        m_i = m_new

    if FINAL:
        denom = tl.where(l_i > 0.0, l_i, 1.0)
        tl.store(out_ptr + (b * HQ + hq) * D + offs_d, acc / denom)
    else:
        base = (pid * NSPLIT + sid) * (D + 2)
        tl.store(part_ptr + base + offs_d, acc)
        tl.store(part_ptr + base + D, m_i)
        tl.store(part_ptr + base + D + 1, l_i)


@triton.jit
def _paged_decode_combine_kernel(
    part_ptr,
    out_ptr,
    D: tl.constexpr,
    NSPLIT: tl.constexpr,
):
    pid = tl.program_id(0)

    offs_s = tl.arange(0, NSPLIT)
    offs_d = tl.arange(0, D)

    base = pid * NSPLIT * (D + 2)

    m_s = tl.load(part_ptr + base + offs_s * (D + 2) + D)
    l_s = tl.load(part_ptr + base + offs_s * (D + 2) + D + 1)

    m_star = tl.max(m_s, axis=0)
    w = tl.exp(m_s - m_star)

    l_tot = tl.sum(w * l_s, axis=0)
    denom = tl.where(l_tot > 0.0, l_tot, 1.0)

    acc = tl.load(
        part_ptr + base + offs_s[:, None] * (D + 2) + offs_d[None, :]
    )
    out = tl.sum(w[:, None] * acc, axis=0) / denom

    tl.store(out_ptr + pid * D + offs_d, out)


def paged_decode_attention(q, k_cache, v_cache, page_table, cache_lens, scale, window, page_size=None):
    if not q.is_cuda or not k_cache.is_cuda or not v_cache.is_cuda:
        raise ValueError("q, k_cache, and v_cache must be CUDA tensors")
    if not page_table.is_cuda or not cache_lens.is_cuda:
        raise ValueError("page_table and cache_lens must be CUDA tensors")

    if q.dim() != 3:
        raise ValueError("q must have shape [B, Hq, D]")
    if k_cache.dim() != 4 or v_cache.dim() != 4:
        raise ValueError("k_cache and v_cache must have shape [num_pages, Hkv, page_size, D]")
    if page_table.dim() != 2:
        raise ValueError("page_table must have shape [B, max_pages_per_seq]")

    B, Hq, D = q.shape
    num_pages, Hkv, P, Dk = k_cache.shape
    num_pages_v, Hkv_v, Pv, Dv = v_cache.shape

    if page_size is None:
        page_size = P

    if P != page_size or Pv != page_size:
        raise ValueError("page_size must match cache tensor page dimension")
    if num_pages != num_pages_v or Hkv != Hkv_v or D != Dk or D != Dv:
        raise ValueError("k_cache/v_cache dimensions must match q")
    if Hq % Hkv != 0:
        raise ValueError("Hq must be divisible by Hkv for GQA")
    if page_table.shape[0] != B:
        raise ValueError("page_table batch dimension must match q")
    if cache_lens.numel() != B:
        raise ValueError("cache_lens must have shape [B]")
    if D not in (64, 128):
        raise ValueError("D must be 64 or 128")
    if page_size not in (8, 16, 32, 64):
        raise ValueError("page_size must be one of 8, 16, 32, 64")

    q_c = q.contiguous()
    k_c = k_cache.contiguous()
    v_c = v_cache.contiguous()
    pt_c = page_table.contiguous()
    if pt_c.dtype != torch.int32:
        pt_c = pt_c.to(torch.int32)
    lens_c = cache_lens.contiguous()
    if lens_c.dtype != torch.int32:
        lens_c = lens_c.to(torch.int32)

    out = torch.empty((B, Hq, D), device=q.device, dtype=torch.float32)

    max_pages = int(pt_c.shape[1])
    max_tokens = int(max_pages * page_size)

    w = int(window)
    nprog = B * Hq

    nsplit = 1
    if w <= 0 and max_tokens >= 1024 and nprog < 128:
        cand = min(8, max_tokens // 512, max(1, 1024 // nprog))
        nsplit = 1 << (cand.bit_length() - 1)

    if nsplit <= 1:
        _paged_decode_attn_kernel[(nprog, 1)](
            q_c,
            k_c,
            v_c,
            pt_c,
            lens_c,
            out,
            out,
            max_tokens,
            Hq,
            Hkv,
            max_pages,
            int(page_size),
            D,
            float(scale),
            w,
            max_tokens,
            NSPLIT=1,
            FINAL=True,
            BLOCK_N=64,
            num_warps=4,
            num_stages=2,
        )
    else:
        split_len = (max_tokens + nsplit - 1) // nsplit
        part = torch.empty(nprog * nsplit * (D + 2), device=q.device, dtype=torch.float32)
        _paged_decode_attn_kernel[(nprog, nsplit)](
            q_c,
            k_c,
            v_c,
            pt_c,
            lens_c,
            out,
            part,
            split_len,
            Hq,
            Hkv,
            max_pages,
            int(page_size),
            D,
            float(scale),
            w,
            max_tokens,
            NSPLIT=nsplit,
            FINAL=False,
            BLOCK_N=64,
            num_warps=4,
            num_stages=2,
        )
        _paged_decode_combine_kernel[(nprog,)](
            part,
            out,
            D,
            NSPLIT=nsplit,
            num_warps=1,
        )

    return out
