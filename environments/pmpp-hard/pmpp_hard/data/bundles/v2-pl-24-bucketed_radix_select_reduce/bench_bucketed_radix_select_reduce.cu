...
static const CaseSpec kCaseSpecs[] = {
    {"uniform_N1M_T16", 1 << 20, 16, BRSR_DIST_UNIFORM},
    {"clustered_N262K_T256", 1 << 18, 256, BRSR_DIST_CLUSTERED},
    {"many_ties_N65536_T4096", 65536, 4096, BRSR_DIST_MANY_TIES},
    {"hot_N1M_T16", 1 << 20, 16, BRSR_DIST_HIGH_BUCKET_HOT},
};
...
// Shape family (N / T / distribution) is fixed per case; only data varies.
    ...
    for (int c = 0; c < kNumCases; ++c) {
        ...
        for (int k = 0; k <= iters; ++k) {
            variants[c].push_back(make_case(
                cs.name, cs.N, cs.T, cs.distribution_id, vs.next_u64()));
        }
    }
...
int main(int argc, char** argv) {
    try {
        ...
        for (int c = 0; c < kNumCases; ++c) {
            ...
            max_T = std::max(max_T, kCaseSpecs[c].T);
        }
        ...
        for (int c = 0; c < kNumCases; ++c) {
            ...
            std::printf(
                "bench_case %-24s N=%d T=%d dist=%d variants=%d\n",
                hc.name.c_str(),
                hc.run.N,
                hc.run.T,
        ...
        // for its own input variant; fold ALL of them (all ten graded fields, fixed
        // order) so a no-op/cached timed call leaves a wrong region and the digest
        ...
        for (int iter = 0; iter < iters; ++iter) {
            for (int c = 0; c < kNumCases; ++c) {
                DeviceCase* dc = cases[c][static_cast<size_t>(iter)];
                const size_t count = (size_t)brsr_count_host(dc->host.run.N, dc->host.run.T);
                dg.dev(dc->threshold_key.ptr, sizeof(uint32_t));
                dg.dev(dc->count.ptr, sizeof(int32_t));
                dg.dev(dc->topT_indices.ptr, count * sizeof(int32_t));
                dg.dev(dc->topT_sum.ptr, sizeof(int64_t));
                dg.dev(dc->topT_max.ptr, sizeof(int32_t));
                dg.dev(dc->topT_argmax.ptr, sizeof(int32_t));
                dg.dev(dc->pass_histograms.ptr,
                       (size_t)BRSR_NUM_PASSES * (size_t)BRSR_BUCKETS * sizeof(int32_t));
                dg.dev(dc->chosen_bucket.ptr, (size_t)BRSR_NUM_PASSES * sizeof(int32_t));
                dg.dev(dc->carried_rank.ptr, (size_t)BRSR_NUM_PASSES * sizeof(int32_t));
                dg.dev(dc->prefix_after_pass.ptr, (size_t)BRSR_NUM_PASSES * sizeof(uint32_t));
            }
        }
        dg.print();
