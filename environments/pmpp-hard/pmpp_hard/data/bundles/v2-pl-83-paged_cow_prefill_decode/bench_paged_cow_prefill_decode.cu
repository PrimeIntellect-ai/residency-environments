...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // timed iteration replays the same fork/prefill/decode SCHEDULE (timing
        // comparable) but on distinct K/V/Q payloads, and its graded outputs are folded
        // into out_fnv after the iteration. A compute-once-then-replay solution serves
        // stale outputs for iterations 2..K → the folded digest diverges → perf FAIL.
        // Per-set seeds come from a SplitMix64 stream over PMPP_BENCH_SEED so the same
        // seed reproduces the same inputs bit-for-bit.
        const uint64_t base_seed = pmpp::bench_seed(0xC0DEC0DEULL);
        ...
        for (int set = 0; set <= iters; ++set) {
            ...
            pool_sets[(size_t)set].resize(kPools);
            std::vector<InputPool>& pools = pool_sets[(size_t)set];
            for (int v = 0; v < kPools; ++v) {
                SplitMix64 rng(set_seed ^ (0xC0DEC0DEULL + (uint64_t)v));
        ...
        for (int iter = 0; iter < iters; ++iter) {
            ...
            dg.dev(d_forks.ptr, sizeof(int32_t));
            dg.dev(d_rels.ptr, sizeof(int32_t));
        }
